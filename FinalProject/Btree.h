...
public:
    ...
    // A function to DeleteNode the key k from the sub-tree rooted with this node
    ...
    // A function to DeleteNode the idx-th key from this node - which is a leaf node
    ...
    // A function to DeleteNode the idx-th key from this node - which is a non-leaf node
    ...
    // Make ] friend of this so that we can access private members of
...
public:
    ...
    void DeleteNode(int k)
    {
        ...
        // Call the DeleteNode function for root
        root->DeleteNode(k);
    ...
    void pointSearch(int id) {
        ...
        if (node == NULL) {
            ...
            fstream myFile;
            ifstream DataFile;
            myFile.open("BTreeStorage/BTreeIn" + to_string(id) + ".txt");
            while (!myFile.eof())
            {
    ...
    void delNode(int id) {
        ...
        if (node == NULL) {
            ...
            string path = "BTreeStorage/BTreeIn" + to_string(id) + ".txt";
            remove(path.c_str());
...
class BTreeNodeString
{
    std::string* keys;  // An array of keys
    int t;      // Minimum degree (defines the range for number of keys)
    BTreeNodeString** C; // An array of child pointers
    int n;     // Current number of keys
    bool leaf; // Is true when node is leaf. Otherwise false

public:

    BTreeNodeString(int t1, bool leaf1)  // Constructor
    {
        // Copy the given minimum degree and leaf property
        t = t1;
        leaf = leaf1;
    ...
    // Function to search key k in subtree rooted with this node
    BTreeNodeString* search(std::string k)
    {
        // Find the first key greater than or equal to k
        int i = 0;
        while (i < n && k > keys[i])
    ...
    // The assumption is, the node must be non-full when this
    // function is called
    void insertNonFull(std::string k)
    {
        // Initialize index as index of rightmost element
        int i = n - 1;
    ...
    // A utility function to split the child y of this node
    // Note that y must be full when this function is called
    void splitChild(int i, BTreeNodeString* y)
    {
        // Create a new node which is going to store (t-1) keys
        // of y
        BTreeNodeString* z = new BTreeNodeString(y->t, y->leaf);
        z->n = t - 1;
    ...
    void borrowFromPrev(int idx)
    {
        ...
        BTreeNodeString* child = C[idx];
        BTreeNodeString* sibling = C[idx - 1];
...
class BTreeString
{
    BTreeNodeString* root; // Pointer to root node
    int t;  // Minimum degree
public:
    ...
    void insert(std::string k)
    {
        ...
        {
            // Allocate memory for root
            root = new BTreeNodeString(t, true);
            root->keys[0] = k;  // Insert key
            root->n = 1;  // Update number of keys in root
        }
        else // If tree is not empty
        {
            ...
            {
                // Allocate memory for new root
                BTreeNodeString* s = new BTreeNodeString(t, false);
    ...
    void pointSearch(std::string id) {
        BTreeNodeString* node = search(id);
        if (node == NULL) {
            cout << "Node-" << id << " not found!" << endl;
        }
        else
        {
            ...
            fstream myFile;
            ifstream DataFile;
            myFile.open("BTreeStorageString/BTreeIn" + id + ".txt");
            while (!myFile.eof())
            {
                getline(myFile, references[0]);
                getline(myFile, references[1]);
    ...
    // function for updating the node
    void updateNode(std::string id, string field, string old_val, string new_val) {
        BTreeNodeString* node = search(id);
        if (node == NULL) {
            cout << "Node-" << id << " not found!" << endl;
        }
    ...
    void delNode(std::string id) {
        ...
        if (node == NULL) {
            ...
            newFile.close();
            // deleting the node form directory
            string path = "BTreeStorageString/BTreeIn" + id + ".txt";
            remove(path.c_str());
            cout << "Node Deleted Successfully!" << endl;
        }
